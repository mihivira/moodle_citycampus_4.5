 ...
 * @return url
 */
function theme_norte19_frontpage_section() {
    ...
    if (!empty($theme->settings->frontpagenavlink)) {
        $templatecontext['frontpagenavlink'] = theme_norte19_header_links($theme->settings->frontpagenavlink, false);
        $templatecontext['frontpagenavlink-m'] = theme_norte19_header_links($theme->settings->frontpagenavlink, true);
    } else {
 ...
 * @return url
 */
function theme_norte19_frontpagecolor() {
    $theme = theme_config::load('norte19');
 ...
 * @return url
 */
function norte19_get_course_image($id) {
    global $CFG;
    $url = '';
 ...
 * @return url
 */
function norte19_get_blog_post_image($id) {
    global $CFG;
    $url = '';
    require_once( $CFG->libdir . '/filelib.php' );
 ...
 * @return url
 */
function norte19_get_user_image($id) {
    global $CFG;
    $url = '';
    require_once( $CFG->libdir . '/filelib.php' );
    $context = context_user::instance( $id );
    $fs = get_file_storage();
    $files = $fs->get_area_files( $context->id, 'user', 'icon', 'norte19', false);
    foreach ($files as $f) {
        if ($f->is_valid_image()) {
 ...
 * @return url
 */
function theme_norte19_frontpageblockcategory($id) {
    GLOBAL  $DB;
 ...
 * @return content.
 */
function theme_norte19_links($links) {
    $weblink = $links;
    $content = "";
    $websettings = explode("\n", $weblink);
 ...
 * @return content.
 */
function theme_norte19_header_links($links, $mobil ) {
    $weblink = $links;
    $content = "";
    $lurl = "";
    $ltxt = "";
 ...
 * @return randcolor
 */
function theme_norte19_random_color() {
    /*
